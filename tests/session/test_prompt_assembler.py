...
def test_stable_cache_key_includes_prompt_digest(tmp_path: Path) -> None:
    ...
    # Key should be: stable_system_prompt:no-skills:<12-char-digest>:<memory-fp>
    parts = cache_keys[0].split(":")
    assert len(parts) == 4
    assert parts[0] == "stable_system_prompt"
    assert len(parts[2]) == 12  # sha256 digest truncated to 12 chars
    assert parts[3] == "no-memory"  # no memory_store, so fingerprint is "no-memory"
...
def test_build_stable_tools_returns_none_when_none(tmp_path: Path) -> None:
    state = make_state(tmp_path)
    assembler = PromptAssembler()

    result = assembler.build_stable_tools(state, tools=None)
...
def test_build_runtime_blocks_omits_file_runtime_when_empty(tmp_path: Path) -> None:
    state = make_state(tmp_path)
    assembler = PromptAssembler()

    blocks = assembler.build_runtime_blocks(state, working_dir=str(tmp_path))

    assert [block.kind for block in blocks] == ["environment", "active_skills", "todo_state"]
